...
import { GameState, Suites, TurnState } from "../../models/enums";
...
export const Game = () => {
    ...
    const onCardDiscarded  = (discarded : Card) => {
        ...
        if(tmpGame && !tmpGame.yourMove) {
            ...
            // if(tmpGame.yourMove) {
            //     const idx = tmpGame.hand?.findIndex((itm: Card) => {
            //         return itm.suite === discarded.suite && itm.value === discarded.value
            //     })

            //     if(idx) {
            //         tmpGame.hand.splice(idx,1);
            //         setHasDiscarded(true);
            //     }
            // }
            ...
            connection
                ...
                .then((result) => {
                    ...
                    connection
                        ...
                        .catch(err => console.error("[GAME] :: " + err.toString()));
    ...
    return game && game.state === GameState.InPlay ? (
        <>
            ...
            <div className="my-cards">
                {
                    game.hand.map((itm: Card, idx: number) => 
                        <CardComponent 
                            key={ `hand-${idx}` } 
                            suite={ itm.suite } 
                            value={ itm.value }
                            click={ () => discard(itm.suite, itm.value) }
                        />
                    )
                }
            </div>
